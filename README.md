...
Contributions must come as merge requests to the develop branch adhere to a single subject, all others will be rejected without comment. By single subject, that is one merge request for one bug fix or feature. Please follow the repository's K&R coding style and formatting conventions, including opening braces on the same line as the statement that introduces the block. I don't care what you think the right way is; you can code that way on your own projects.

I use the GitLab issue board and milestones.  Please only contribute issues slated for the next milestone (when one is planned), and put
